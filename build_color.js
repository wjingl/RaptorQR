...
s1 = patchWorker(s1, 'decode', (code) => {
  ...
  code = code.replace(ys,
    'async function ys(r,epoch){if(epoch!==void 0&&epoch!==queueEpoch)return;var mc,cp=[],n=[],qrSource="raw";' +
    // maybeColor 只作为顺序优化提示：黑白帧先走标准 QR，标准 QR 失败后仍继续彩色解析。
    'try{if(!r||!r.data||!r.width||!r.height)return;mc=CimQR.maybeColor(r.data,r.width,r.height);const alive=()=>epoch===void 0||epoch===queueEpoch;const accept=async(list,standard,source)=>{if(!alive()||!list||!list.length)return false;packetStats.packetsDecoded+=list.length;try{var inf=standard?{stage:"qr-standard",format:"qr-standard",symbols:list.length,source:source,version:list[0]&&list[0].version||0}:{...CimQR.info(),source:source};inf.symbols=list.length;inf.symbolsPerFrame=list.length;self.postMessage({type:"single-code",info:inf,color:!standard})}catch{}let accepted=0;for(const item of list){if(!alive())return false;let packet;try{packet=standard?Di(item.bytes):Di(item)}catch{packetStats.packetsCRCRejected++;continue}if(alive()&&await ws(standard?item:{version:0},packet,accepted===0)&&(accepted++,b!=null&&b.completed)){Vr(b);return true}}if(b&&accepted>0&&alive())Vr(b);return accepted>0};' +
    'if(!mc){n=await mi(r,{..._t,maxSymbols:vs()});if(!alive())return;if(await accept(n,true,"raw"))return;n=[];}' +
    'try{if(!alive())return;cp=CimQR.decode(r.data,r.width,r.height)||[]}catch(e2){cp=[]}' +
    'try{if(alive())self.postMessage({type:"single-code",info:CimQR.info(),color:cp.length>0||mc})}catch(e3){}' +
    'if(alive()&&cp.length&&await accept(cp,false,"full"))return;' +
    'if(mc){n=await mi(r,{..._t,maxSymbols:vs()});if(!alive())return;}' +
    'if(n.length===0){try{if(!alive())return;const e=CimQR.enhance(r.data,r.width,r.height);qrSource="enhanced";n=await mi({data:e.data,width:e.width,height:e.height},{..._t,maxSymbols:vs()})}catch(e4){}}' +
    'if(!alive())return;if(n.length===0){try{self.postMessage({type:"single-code",info:{stage:mc?"color-parse-failed":"qr-no-result",format:mc?"color-cimbar":"qr-standard",symbols:0,source:qrSource},color:mc})}catch(e5){}return}' +
    'if(alive())await accept(n,true,qrSource);' +
  ...
  // 仅合并当前在途队列中的相同实时帧；失败帧不写入历史缓存，仍允许后续重试
  ...
  code = code.replace(wsGuard,
    'async function ws(r,n,i){if(!n||!guardHeader(n.header,n.payload&&n.payload.length)){packetStats.packetsHeaderRejected++;if(!hr){hr=!0;self.postMessage({type:"error",message:"FEC header rejected: limits or metadata invalid"})}return false}const u=ki(n.header);return $s(u)?u==="wasm-raptorq"?_s(r,n,i):bs(r,n,i):(lr(u),!1)}');
  code = code.replace('function bs(r,n,i){const u=n.header;',
    'function bs(r,n,i){const u=n.header;if(b&&((b.codec!=="js-rlnc")||b.dataLength!==u.dataLength||b.totalGenerations!==u.totalGenerations||b.symbolSize!==n.payload.length||b.isText!==u.isText||b.isCompressed!==u.compressed)){packetStats.packetsMetadataRejected++;return false;}');
  code = code.replace('function _s(r,n,i){const u=n.header;',
    'function _s(r,n,i){const u=n.header;if(b&&((b.codec!=="wasm-raptorq")||b.dataLength!==u.dataLength||b.totalGenerations!==u.totalGenerations||b.symbolSize!==n.payload.length||b.isText!==u.isText||b.isCompressed!==u.compressed)){packetStats.packetsMetadataRejected++;return false;}');
