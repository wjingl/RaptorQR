...
(async () => {
  // 随机 30KB 文件
  const FILE = process.cwd().replace(/\\/g, '/') + '/rand_30k.bin';
  const rnd = Buffer.alloc(30000);
  for (let i = 0; i < rnd.length; i++) rnd[i] = (Math.random() * 256) | 0;
  fs.writeFileSync('rand_30k.bin', rnd);
  ...
  if (result.frames && result.frames.length > 1) {
    fs.writeFileSync('cdp_multipacket.json', JSON.stringify({ frames: result.frames, expectedSize: result.expectedSize }));
